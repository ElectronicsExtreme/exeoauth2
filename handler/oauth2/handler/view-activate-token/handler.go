...
import (
	...
	"dev.corp.extreme.co.th/exe-account/account-interface/handler/oauth2/handler/view-activate-token/response-writer"
	"dev.corp.extreme.co.th/exe-account/account-interface/httputilextend"
)

const (
	RequireScopes = "view_activate_token"
)

var (
	PrefixPath       = oauth2.PrefixPath + "/view_activate_token"
	validateTokenURI = ""
...
func (*Handler) ServeHTTP(httpResp http.ResponseWriter, req *http.Request) {
	...
	if userInfo.EmailActivated {
		resp.WriteResult(&response.ErrorAccountActivated)
		return
	}
	success.ActivationToken, err = user.GetEmailActivationString(userInfo)
	if err != nil {
		logger, errr := errorLogger()
		if errr != nil {
			log.Println(PrefixPath+" :", errr)
